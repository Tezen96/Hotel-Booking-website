...
if (isset($_POST['update'])) {
    ...
    $facilities = isset($_POST['facilities']) ? mysqli_real_escape_string($conn, $_POST['facilities']) : $_POST['facilities'];
    $features = isset($_POST['features']) ? mysqli_real_escape_string($conn, $_POST['features']) : $_POST['features'];
    $price = isset($_POST['price']) ? mysqli_real_escape_string($conn, $_POST['price']) : $_POST['price'];
    ...
    if (!empty($_FILES["image"]["name"])) {
        ...
        $sql = "SELECT image FROM room WHERE id = $id";
    ...
    $sql = "UPDATE room SET name = '{$name}', description = '{$description}', image = '{$image}', facilities = '{$facilities}', features = '{$features}', price = '{$price}' WHERE id = $id ";
    ...
    while ($row2 = mysqli_fetch_assoc($result2)) {
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div class="input-group mb-5">
                ...
                <input type="number" value="<?php echo $row2['price']; ?>" name="price" class="form-control" placeholder="" aria-label="Username" aria-describedby="basic-addon1">
            </div>

            <div class="form-floating mb-5">
                <textarea name="features" class="form-control" placeholder="Leave a comment here" id="floatingTextarea2" style="height: 200px"><?php echo $row2['features']; ?></textarea>
                <label for="floatingTextarea2">Features</label>
            </div>

            <div class="input-group mb-5">
                <span class="input-group-text" id="basic-addon1">Facilities</span>
                <input name="facilities" type="text" value="<?php echo $row2['facilities']; ?>" class="form-control" placeholder="" aria-label="Username" aria-describedby="basic-addon1">
            </div>
